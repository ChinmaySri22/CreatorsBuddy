...
class ScriptValidator:
    ...
    def _analyze_tone_markers(self, text: str) -> Dict[str, float]:
        ...
        enthusiasm_score = sum(1 for word in enthusiasm_words if word in text_lower) / len(text.split()) * 100
    ...
    def _calculate_overall_score(self, metrics: Dict, validation_results: Dict) -> float:
        ...
        scores = {
            'length_appropriate': max(0, 1 - metrics['length_deviation']),
            'has_structure': metrics['structure_score'],
            'engagement_ok': min(1, validation_results['engagement_markers'] / self.thresholds['min_engagement_words']),
            'readability_good': min(1, metrics['readability_score'] / 100),
            'language_mix_ok': min(1, metrics['language_mix']['mixed_ratio'] / self.thresholds['min_hinglish_ratio'])
        }
